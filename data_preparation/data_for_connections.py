...
CONNECTIONS_OUTPUT_FILEPATH = '/Users/erykmroczko/Downloads/connections_data.csv'

airports_df = pd.read_csv(AIRPORTS_FILEPATH, usecols=AIRPORTS_FILE_COLUMNS).dropna()
airports_df.rename(columns={'Name': 'AIRPORT_NAME', 'City': 'CITY', 'Country': 'COUNTRY'}, inplace=True)
...
flights_df.rename(
    columns={'AIRPORT_NAME': 'ORIGIN_AIRPORT_NAME', 'CITY': 'ORIGIN_CITY', 'COUNTRY': 'ORIGIN_COUNTRY'}, inplace=True)
...
flights_df.rename(columns={'AIRPORT_NAME': 'DESTINATION_AIRPORT_NAME', 'CITY': 'DESTINATION_CITY',
                           'COUNTRY': 'DESTINATION_COUNTRY'}, inplace=True)
flights_df = flights_df.drop(['IATA'], axis=1)

flights_df = flights_df.groupby(['ORIGIN_CITY', 'DESTINATION_CITY']).size().reset_index(name='NUMBER_OF_FLIGHTS')
